...
import vip.chentianxiang.middleware.mybatis.boot.mybatis.SqlSessionFactoryBuilder;
import vip.chentianxiang.middleware.mybatis.boot.spring.MapperFactoryBean;
import vip.chentianxiang.middleware.mybatis.boot.spring.MapperScannerConfigurer;
 ...
 * @Description: 加载配置和初始化 Bean
 *
 *- @EnableConfigurationProperties(MybatisProperties.class)，用于引入配置信息，如果你不引入是不能使用配置信息的。
 *- Connection connection(MybatisProperties mybatisProperties)，实例化数据库连接，mybatisProperties 是这个方法的入参，
 * 这里也可以通过属性注入方法体里使用的方式进行处理。另外，这里返回的数据库连接方式，是我们从类 SqlSessionFactoryBuilder 提取出来的，
...
public class MybatisAutoConfiguration implements InitializingBean {
    ...
    /**
     * MapperScannerRegistrarNotFoundConfiguration，它的作用是把我们的用于扫描加载配置和初始化的类启动起来。
     *
     * 其实它的方式也非常简单，主要通过引入 @Import(AutoConfiguredMapperScannerRegistrar.class) 在实现了 InitializingBean 类上，就可以被 Spring 处理了。
     */
    @Configuration
